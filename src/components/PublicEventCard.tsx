...
function getTimeLeft(target: Date): TimeLeft {
  ...
  if (difference <= 0 || difference > THRESHOLD_MS) { return null; }
...
function useCountdown(
  ...
  const [timeLeft, setTimeLeft] =
    useState<TimeLeft>(() =>
      isoString
        ? getTimeLeft(new Date(isoString))
        : null,
    );
...
function Colon() {
  return (
    <span className="mb-4 text-sm font-bold text-countdown-separator">
      :
    </span>
  );
}
    ...
    event.language === "en"
      ...
      : event.language === "sv"
        ? {
            label: t("langSv"),
            className:
              "bg-lang-sv text-lang-sv-foreground",
          }
        : event.language === "both"
          ? {
              label: t("langBoth"),
              className:
                "bg-lang-both text-lang-both-foreground",
            }
          : undefined;
